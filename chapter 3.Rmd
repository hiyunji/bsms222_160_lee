...
The formula for the sum of the series  1+2+⋯+n
  is  n(n+1)/2. What if we weren’t sure that was the right function? How could we check? Using what we learned about functions we can create one that computes the S_n:
```{r}
...
How can we compute S_n 
  for various values of n, say  n=1,...,25? Do we write 25 lines of code calling `compute_s_n`? No, that is what for-loops are for in programming. In this case, we are performing exactly the same task over and over, and the only thing that is changing is the value of n
...
```{r}
altman_plot<-function(x,y){plot(x+y,abs(x-y))}
```
...
n computes the sum S_n = 1^2 + 2^2 + 3^2 +...+ n^2  . Report the value of the sum when n=10.
```{r}
