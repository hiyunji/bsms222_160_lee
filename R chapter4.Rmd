...
```{r}
pander(filter(murders, state == "New York"))
```
...
```{r}
pander(filter(murders, state %in% c("New York", "Texas")))
```
...
```{r}
pander(filter(murders, population < 5000000 & region == "Northeast"))
```
...
pander(select(my_states,state, rate, rank))
```
##4.5 The pipe: `%>%`
...
```{r}
pander(murders %>% select(state, region, rate) %>% filter(rate <= 0.71))
```
...
```{r}
pander(mutate(murders, rate =  total / population * 100000, 
       rank = rank(-rate)) %>%
...
```{r}
pander(mutate(murders,rate= total/population*100000,rank=rank(-rate)) %>% filter(region%in%c("Northeast","West")&rate<1)%>% select(state,rate,rank))
```
