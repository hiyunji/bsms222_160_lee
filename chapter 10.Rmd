...
murders %>% mutate(murder_rate = total / population * 100000) %>%
 ...
 xlab("") +theme(axis.text.y = element_text(size = 6))
```
...
heights %>% 
  ...
  geom_histogram(binwidth = 1, color="black") +facet_grid(sex~.)
...
us_contagious_diseases %>% filter(disease=="Smallpox" & weeks_reporting >=10) %>% mutate(rate=count/population*10000*52/weeks_reporting)%>% mutate(state = reorder(state, rate)) %>% ggplot(aes(year, state, fill = rate)) + 
  geom_tile(color = "grey50") + 
  scale_x_continuous(expand=c(0,0)) + 
  scale_fill_gradientn(colors = brewer.pal(9, "Reds"), trans = "sqrt") + 
  theme_minimal() + 
...
us_contagious_diseases %>% filter(disease=="Smallpox" & weeks_reporting >=10) %>% mutate(rate=count/population*10000*52/weeks_reporting)%>% mutate(state = reorder(state, rate)) %>% filter(!is.na(rate)) %>%
    ggplot() +
  geom_line(aes(year, rate, group = state),  color = "grey50", 
            show.legend = FALSE, alpha = 0.2, size = 1) +
  geom_line(mapping = aes(year, us_rate),  data = avg, size = 1) +
  scale_y_continuous( trans="sqrt",breaks = c(5,10)) + 
  ggtitle("Cases per 10,000 by state") + 
  xlab("") + ylab("") +
  geom_text(data = data.frame(x = 1940, y = 3), 
            mapping = aes(x, y, label="US average"), 
            color="black")
```
...
```{r}
us_contagious_diseases%>%filter(state=="California"&weeks_reporting >=10)%>% mutate(rate=count/population*10000*52/weeks_reporting)%>% mutate(disease = reorder(disease, rate))%>%ggplot(aes(year,rate,col=disease))+geom_line()+scale_y_continuous( trans="sqrt",breaks = c(5,25,125))
```
...
```{r}
us_contagious_diseases %>%group_by(year, disease)%>% summarize(us_rate = sum(count, na.rm = TRUE) / 
              sum(population, na.rm = TRUE) * 10000) %>% ggplot(aes(year, us_rate, col=disease)) + geom_line()+scale_y_continuous( trans="sqrt",breaks = c(5,25,50))
```
